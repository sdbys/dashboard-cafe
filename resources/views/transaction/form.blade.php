                ...
                @foreach ($dtMenu as $rsMenu)
                    <div class="menu-item col-md-3" onclick="add_menu('{{ $rsMenu->id }}','{{ $rsMenu->mn_nama }}','{{ $rsMenu->mn_harga }}')">
                        <div class="inner">
                            ...
                            @endif
                            <h2>{{ $rsMenu->mn_nama }} <br/><span>{{ number_format($rsMenu->mn_harga,"0",",",".") }}</span></h2>
                        </div>
                    </div>
                @endforeach
                </div>
        ...
        <div class="col-md-4">
            <div class="transaksi">
                <form id="frmTransaksi" action="{{ url("transaction/save") }}" method="post">
                    ...
                    <p>
                        <input type="text" name="meja" id="meja" placeholder="Meja / Atas Nama">
                    </p>
                    {{-- End Info Transaksi --}}
                    <hr>
                    ...
                    <div class="row btn-action">
                        <div class="col-md-6">
                            <button id="btn-save" class="btn btn-success" type="button" onclick="save_transaksi()" data-url="{{ url("transaction/nota") }}">SAVE</button>                            
                            <button id="btn-new" class="btn btn-primary d-none" type="button" onclick="new_transaksi()" >NEW TRANSAKSI</button>                            
                        </div>
                        <div class="col-md-6">
